...
import com.praetor.submission.engine.checker.ExactChecker;
import org.junit.jupiter.api.Test;

class VerdictEvaluatorTest {

    private final VerdictEvaluator ev = new VerdictEvaluator();
    private final Checker exact = new ExactChecker();
    ...
    void ac_on_exact_match() {
        assertThat(ev.evaluate(cleanExit("5\n", 10, 2000), "5", limits, exact)).isEqualTo("AC");
    }
    ...
    @Test
    void wa_on_mismatch() {
        assertThat(ev.evaluate(cleanExit("6", 10, 2000), "5", limits, exact)).isEqualTo("WA");
    }

    @Test
    void tle_when_timed_out() {
        assertThat(ev.evaluate(new RunResult(null, "", 2000, null, true, false), "5", limits, exact))
                .isEqualTo("TLE");
    }

    @Test
    void tle_when_finished_over_soft_limit() {
        assertThat(ev.evaluate(cleanExit("5", 1500, 2000), "5", limits, exact)).isEqualTo("TLE");
    }

    @Test
    void mle_when_peak_mem_exceeds_limit() {
        assertThat(ev.evaluate(cleanExit("5", 10, 300_000), "5", limits, exact)).isEqualTo("MLE");
    }

    @Test
    void re_on_nonzero_exit() {
        assertThat(ev.evaluate(new RunResult(139, "", 10, 2000, false, false), "5", limits, exact))
                .isEqualTo("RE");
    }

    @Test
    void re_on_output_flood_truncation() {
        assertThat(ev.evaluate(new RunResult(0, "huge", 10, 2000, false, true), "5", limits, exact))
                .isEqualTo("RE");
    }

    @Test
    void mle_on_137_without_timeout() {
        assertThat(ev.evaluate(new RunResult(137, "", 10, null, false, false), "5", limits, exact))
                .isEqualTo("MLE");
    }
}
